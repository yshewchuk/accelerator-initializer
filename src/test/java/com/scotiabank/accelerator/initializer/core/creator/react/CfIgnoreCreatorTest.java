...
public class CfIgnoreCreatorTest {
    ...
    public void before() {
        ...
        this.creator = new CfIgnoreCreator(fileProcessor);
    }

    @Test
    public void assertItLoadsCfIgnoreTemplate() {
        ProjectCreation request = ProjectCreation.builder().rootDir(".").build();
        creator.create(request);
        verify(this.fileProcessor, times(1)).loadResourceFromClassPath(CfIgnoreCreator.CF_IGNORE_TEMPLATE_PATH);
    }
    ...
    public void assertItCreateFile() {
        ProjectCreation request = ProjectCreation.builder().rootDir(".").build();
        creator.create(request);
        verify(this.fileProcessor, times(1)).touch(Paths.get("./.cfignore"));
    }
    
    @Test
    public void assertItCopiesContentToFile() throws IOException {
        File f = folder.newFile(".cfignore");
