...
public class EslintrcCreatorTest {
    ...
    public void before() {
        ...
        this.creator = new EslintrcCreator(fileProcessor);
    }

    @Test
    public void assertItLoadsEslintrcTemplate() {
        ProjectCreation request = ProjectCreation.builder().rootDir(".").build();
        creator.create(request);
        verify(this.fileProcessor, times(1)).loadResourceFromClassPath(EslintrcCreator.ESLINTRC_TEMPLATE_PATH);
    }
    ...
    public void assertItCreateFile() {
        ProjectCreation request = ProjectCreation.builder().rootDir(".").build();
        creator.create(request);
        verify(this.fileProcessor, times(1)).touch(Paths.get("./.eslintrc"));
    }
    
    @Test
    public void assertItCopiesContentToFile() throws IOException {
        File f = folder.newFile(".eslintrc");
